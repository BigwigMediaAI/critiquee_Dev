...
function PreviewCard({ content, imageUrls, businessName, mode }) {
  ...
  return (
    <div
      className={`${containerWidth} mx-auto bg-white dark:bg-zinc-900 rounded-xl border border-border shadow-sm overflow-hidden`}
    >
      {/* Header */}
      ...
      {content && (
        <div className="px-4 py-3">
          <p
            className={`whitespace-pre-wrap break-words text-foreground ${isMobile ? "text-sm" : "text-[15px]"} leading-relaxed`}
          >
            {content}
          </p>
        </div>
      ...
      {imageUrls && imageUrls.length > 0 && (
        <div
          className={`grid gap-0.5 ${imageUrls.length === 1 ? "grid-cols-1" : "grid-cols-2"}`}
        >
...
export default function PostPreviewDialog({
  ...
  const imageUrls = (images || [])
    .map((img) => img.preview || resolveImageUrl(img.url))
    .filter(Boolean);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[95vw] max-w-[95vw] sm:max-w-2xl max-h-[90vh] overflow-y-auto overflow-x-hidden p-3 sm:p-6">
        <DialogHeader>
          <DialogTitle data-testid="post-preview-title">
        ...
        <div className="flex items-center gap-2 p-1 rounded-xl bg-muted/40 w-fit mx-auto">
          <button
            ...
            className={`w-full sm:w-auto justify-center flex items-center gap-2 px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${
              mode === "desktop"
        ...
        <div className="py-4 px-1 sm:px-4 flex justify-center bg-gradient-to-b from-muted/30 to-transparent rounded-xl overflow-x-hidden">
          <PreviewCard
            content={content}
            imageUrls={imageUrls}
