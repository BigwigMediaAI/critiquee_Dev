  ...
  const handleExport = async (kind) => {
    ...
    try {
      const fnMap = {
        ...
        audit: () =>
          reportsApi.exportAuditLogs({ date_from: dateFrom, date_to: dateTo }),
      };
      const res = await fnMap[kind]();
      const blob = new Blob([res.data], { type: "text/csv" });
      downloadBlob(
        blob,
        `${kind}_${new Date().toISOString().slice(0, 10)}.csv`,
      );
      toast.success(
        `${kind === "audit" ? "Audit logs" : kind[0].toUpperCase() + kind.slice(1)} exported`,
      );
    } catch (e) {
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {periodLabel}
            {platform !== "all" && ` · ${getPlatformLabel(platform)}`}
          </p>
        </div>
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="min-w-[220px]">
            ...
            >
              <BarChart2 size={14} className="mr-2 text-primary" /> Summary
              report
            </DropdownMenuItem>
            <DropdownMenuItem
              onClick={() => handleExport("reviews")}
              data-testid="export-reviews"
            >
            ...
            >
              <MessageSquare size={14} className="mr-2 text-violet-500" />{" "}
              Social comments
            </DropdownMenuItem>
            <DropdownMenuItem
            ...
            >
              <Activity size={14} className="mr-2 text-emerald-500" /> Activity
              log
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      ...
      <Card>
        <CardContent className="pt-4 pb-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-3">
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground sm:col-span-2 xl:col-span-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          icon={MessageSquare}
          color="bg-violet-500"
          sub={`${summary?.social?.replied_comments || 0} replied`}
        />
      </div>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle
              ...
              style={{ fontFamily: "Manrope" }}
            >
              <Activity size={16} className="text-primary" /> Review Trends (14
              days)
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle
              ...
              style={{ fontFamily: "Manrope" }}
            >
              <BarChart2 size={16} className="text-amber-500" /> Rating
              Distribution
            </CardTitle>
          </CardHeader>
          <CardContent>
          ...
          <CardContent>
            <div className="space-y-3">
              {platformData.map((p, i) => {
                const pct = summary?.reviews?.total
                  ? Math.round((p.count / summary.reviews.total) * 100)
                  : 0;
                return (
                  <div
                    key={p.name}
                  ...
                  >
                    ...
                    <span className="text-sm text-foreground sm:w-24 break-words">
                      {p.name}
                    </span>
          ...
          {logs.length === 0 ? (
            ...
            <div className="space-y-2">
              {logs.map((log) => (
                <div
                  key={log.id}
                  className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 py-2border-b border-border last:border-0"
                >
